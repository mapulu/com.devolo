...
class DevoloSiren extends ZwaveDevice {
	...
	async onMeshInit() {
       ...
       this.registerReportListener('SENSOR_BINARY', 'SENSOR_BINARY_REPORT', report => {
            ...
            if (report['Sensor Type'] === 'Tamper' && report['Sensor Value'] === 'detected an event') {
		     ...
		     this.PlaySoundFlow = new Homey.FlowCardAction('MT2758play_sound').register().registerRunListener((args, state) => {
			...
			this.DisableSirenFlow = new Homey.FlowCardAction('MT2758disable_siren').register().registerRunListener((args, state) => {
			return this.configurationSet({
                index: 29,
                size: 1,
			}, 1)
		    });		
			this.EnableSirenFlow = new Homey.FlowCardAction('MT2758enable_siren').register().registerRunListener((args, state) => {
			return this.configurationSet({
                index: 29,
                size: 1,
			}, 0)
